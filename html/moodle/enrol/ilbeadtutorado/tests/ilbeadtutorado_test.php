...
global $CFG;
require_once($CFG->dirroot.'/enrol/ilbeadtutorado/lib.php');
require_once($CFG->dirroot.'/enrol/ilbeadtutorado/locallib.php');

class enrol_ilbeadtutorado_testcase extends advanced_testcase {

    public function test_basics() {
        $this->assertTrue(enrol_is_enabled('ilbeadtutorado'));
        $plugin = enrol_get_plugin('ilbeadtutorado');
        $this->assertInstanceOf('enrol_ilbeadtutorado_plugin', $plugin);
        $this->assertEquals(1, get_config('enrol_ilbeadtutorado', 'defaultenrol'));
        $this->assertEquals(ENROL_EXT_REMOVED_KEEP, get_config('enrol_ilbeadtutorado', 'expiredaction'));
    }

    public function test_sync_nothing() {
        global $SITE;

        $ilbeadtutoradoplugin = enrol_get_plugin('ilbeadtutorado');

        $trace = new null_progress_trace();

        // Just make sure the sync does not throw any errors when nothing to do.
        $ilbeadtutoradoplugin->sync($trace, null);
        $ilbeadtutoradoplugin->sync($trace, $SITE->id);
    }
    ...
    public function test_longtimnosee() {
        ...
        $this->assertEquals(3, $DB->count_records('enrol', array('enrol'=>'ilbeadtutorado')));
        $instance1 = $DB->get_record('enrol', array('courseid'=>$course1->id, 'enrol'=>'ilbeadtutorado'), '*', MUST_EXIST);
        $instance2 = $DB->get_record('enrol', array('courseid'=>$course2->id, 'enrol'=>'ilbeadtutorado'), '*', MUST_EXIST);
        $instance3 = $DB->get_record('enrol', array('courseid'=>$course3->id, 'enrol'=>'ilbeadtutorado'), '*', MUST_EXIST);
        $id = $ilbeadtutoradoplugin->add_instance($course3, array('status'=>ENROL_INSTANCE_ENABLED, 'roleid'=>$teacherrole->id));
        ...
        $ilbeadtutoradoplugin->enrol_user($instance1, $user1->id, $studentrole->id);
        $ilbeadtutoradoplugin->enrol_user($instance1, $user2->id, $studentrole->id);
        $ilbeadtutoradoplugin->enrol_user($instance1, $user3->id, $studentrole->id);
        ...
        $ilbeadtutoradoplugin->enrol_user($instance3, $user1->id, $studentrole->id);
        $ilbeadtutoradoplugin->enrol_user($instance3, $user2->id, $studentrole->id);
        $ilbeadtutoradoplugin->enrol_user($instance3, $user3->id, $studentrole->id);
        $ilbeadtutoradoplugin->enrol_user($instance3b, $user1->id, $teacherrole->id);
        $ilbeadtutoradoplugin->enrol_user($instance3b, $user4->id, $teacherrole->id);
        ...
        $ilbeadtutoradoplugin->sync($trace, $course2->id);
        $this->assertEquals(10, $DB->count_records('user_enrolments'));
    ...
    public function test_expired() {
        ...
        $ilbeadtutoradoplugin->enrol_user($instance1, $user1->id, $studentrole->id);
        $ilbeadtutoradoplugin->enrol_user($instance1, $user2->id, $studentrole->id);
        $ilbeadtutoradoplugin->enrol_user($instance1, $user3->id, $studentrole->id, 0, $now-60);

        $ilbeadtutoradoplugin->enrol_user($instance3, $user1->id, $studentrole->id, 0, 0);
        $ilbeadtutoradoplugin->enrol_user($instance3, $user2->id, $studentrole->id, 0, $now-60*60);
        $ilbeadtutoradoplugin->enrol_user($instance3, $user3->id, $studentrole->id, 0, $now+60*60);
        $ilbeadtutoradoplugin->enrol_user($instance3b, $user1->id, $teacherrole->id, $now-60*60*24*7, $now-60);
        $ilbeadtutoradoplugin->enrol_user($instance3b, $user4->id, $teacherrole->id);
        ...
        $this->assertEquals(ENROL_EXT_REMOVED_KEEP, $ilbeadtutoradoplugin->get_config('expiredaction'));
        $ilbeadtutoradoplugin->sync($trace, null);
        $this->assertEquals(10, $DB->count_records('user_enrolments'));
        $this->assertEquals(10, $DB->count_records('role_assignments'));


        $ilbeadtutoradoplugin->set_config('expiredaction', ENROL_EXT_REMOVED_SUSPENDNOROLES);
        $ilbeadtutoradoplugin->sync($trace, $course2->id);
        $this->assertEquals(10, $DB->count_records('user_enrolments'));
        $this->assertEquals(10, $DB->count_records('role_assignments'));
        ...
        $ilbeadtutoradoplugin->set_config('expiredaction', ENROL_EXT_REMOVED_UNENROL);
    ...
    public function test_send_expiry_notifications() {
        ...
        /** @var $ilbeadtutoradoplugin enrol_ilbeadtutorado_plugin */
        $ilbeadtutoradoplugin = enrol_get_plugin('ilbeadtutorado');
        ...
        $ilbeadtutoradoplugin->set_config('expirynotifylast', $now - 60*60*24);
        $ilbeadtutoradoplugin->set_config('expirynotifyhour', 0);
        ...
        $this->assertEquals(4, $DB->count_records('enrol', array('enrol'=>'ilbeadtutorado')));
        ...
        $instance4 = $DB->get_record('enrol', array('courseid'=>$course4->id, 'enrol'=>'ilbeadtutorado'), '*', MUST_EXIST);
        ...
        $ilbeadtutoradoplugin->enrol_user($instance1, $user1->id, $studentrole->id, 0, $now + 60*60*24*1, ENROL_USER_SUSPENDED); // Suspended users are not notified.
        $ilbeadtutoradoplugin->enrol_user($instance1, $user2->id, $studentrole->id, 0, $now + 60*60*24*5);                       // Above threshold are not notified.
        $ilbeadtutoradoplugin->enrol_user($instance1, $user3->id, $studentrole->id, 0, $now + 60*60*24*3 + 60*60);               // Less than one day after threshold - should be notified.
        $ilbeadtutoradoplugin->enrol_user($instance1, $user4->id, $studentrole->id, 0, $now + 60*60*24*4 - 60*3);                // Less than one day after threshold - should be notified.
        $ilbeadtutoradoplugin->enrol_user($instance1, $user5->id, $studentrole->id, 0, $now + 60*60);                            // Should have been already notified.
        $ilbeadtutoradoplugin->enrol_user($instance1, $user6->id, $studentrole->id, 0, $now - 60);                               // Already expired.
        ...
        $ilbeadtutoradoplugin->enrol_user($instance2, $user1->id, $studentrole->id);
        $ilbeadtutoradoplugin->enrol_user($instance2, $user2->id, $studentrole->id, 0, $now + 60*60*24*1 + 60*3);                // Above threshold are not notified.
        $ilbeadtutoradoplugin->enrol_user($instance2, $user3->id, $studentrole->id, 0, $now + 60*60*24*1 - 60*60);               // Less than one day after threshold - should be notified.

        $manualplugin->enrol_user($maninstance3, $user1->id, $editingteacherrole->id);
        $ilbeadtutoradoplugin->enrol_user($instance3, $user2->id, $studentrole->id, 0, $now + 60*60*24*1 + 60);                  // Above threshold are not notified.
        $ilbeadtutoradoplugin->enrol_user($instance3, $user3->id, $studentrole->id, 0, $now + 60*60*24*1 - 60*60);               // Less than one day after threshold - should be notified.

        $manualplugin->enrol_user($maninstance4, $user4->id, $editingteacherrole->id);
        $ilbeadtutoradoplugin->enrol_user($instance4, $user5->id, $studentrole->id, 0, $now + 60*60*24*1 + 60);
        $ilbeadtutoradoplugin->enrol_user($instance4, $user6->id, $studentrole->id, 0, $now + 60*60*24*1 - 60*60);
        ...
        $ilbeadtutoradoplugin->send_expiry_notifications($trace);
        ...
        $ilbeadtutoradoplugin->set_config('expirynotifylast', time() - 60*60*24);
        $ilbeadtutoradoplugin->set_config('expirynotifyhour', '24');
    ...
    public function test_show_enrolme_link() {
        ...
        $DB->update_record('enrol', $instance1);
        $ilbeadtutoradoplugin->update_status($instance1, ENROL_INSTANCE_ENABLED);

        $instance2 = $DB->get_record('enrol', array('courseid'=>$course2->id, 'enrol'=>'ilbeadtutorado'), '*', MUST_EXIST);
        $instance2->customint6 = 0;
        $DB->update_record('enrol', $instance2);
        $ilbeadtutoradoplugin->update_status($instance2, ENROL_INSTANCE_ENABLED);

        $instance3 = $DB->get_record('enrol', array('courseid'=>$course3->id, 'enrol'=>'ilbeadtutorado'), '*', MUST_EXIST);
        $instance3->customint6 = 1;
        $DB->update_record('enrol', $instance3);
        $ilbeadtutoradoplugin->update_status($instance3, ENROL_INSTANCE_DISABLED);

        $instance4 = $DB->get_record('enrol', array('courseid'=>$course4->id, 'enrol'=>'ilbeadtutorado'), '*', MUST_EXIST);
        $instance4->customint6 = 0;
        $DB->update_record('enrol', $instance4);
        $ilbeadtutoradoplugin->update_status($instance4, ENROL_INSTANCE_DISABLED);

        $instance5 = $DB->get_record('enrol', array('courseid'=>$course5->id, 'enrol'=>'ilbeadtutorado'), '*', MUST_EXIST);
        $instance5->customint6 = 1;
        $instance5->customint5 = $cohort1->id;
        $DB->update_record('enrol', $instance1);
        $ilbeadtutoradoplugin->update_status($instance5, ENROL_INSTANCE_ENABLED);

        $id = $ilbeadtutoradoplugin->add_instance($course5, $ilbeadtutoradoplugin->get_instance_defaults());
        $instance6 = $DB->get_record('enrol', array('id'=>$id), '*', MUST_EXIST);
        $instance6->customint6 = 1;
        $instance6->customint5 = $cohort2->id;
        $DB->update_record('enrol', $instance1);
        $ilbeadtutoradoplugin->update_status($instance6, ENROL_INSTANCE_ENABLED);

        $this->setUser($user1);
        $this->assertTrue($ilbeadtutoradoplugin->show_enrolme_link($instance1));
        $this->assertFalse($ilbeadtutoradoplugin->show_enrolme_link($instance2));
        $this->assertFalse($ilbeadtutoradoplugin->show_enrolme_link($instance3));
        $this->assertFalse($ilbeadtutoradoplugin->show_enrolme_link($instance4));

        require_once("$CFG->dirroot/cohort/lib.php");
        cohort_add_member($cohort1->id, $user1->id);

        $this->assertTrue($ilbeadtutoradoplugin->show_enrolme_link($instance5));
        $this->assertFalse($ilbeadtutoradoplugin->show_enrolme_link($instance6));
    }
}
